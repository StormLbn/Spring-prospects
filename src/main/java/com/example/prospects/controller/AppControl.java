...
import com.example.prospects.services.ProspectService;
...
public class AppControl {
	...
	public String index(Model model) {
		...
		// Création d'un objet User vide et injection dans la page HTML pour qu'il puisse être utilisé dans le formulaire de connexion
		User user = new User();
	...
	public String clientList(Model model) {
		...
		model.addAttribute("clients", prospectServices.getData());
		return "clientList";
	}
	...
	public String showUpdateClientPage(Model model, @PathVariable(value = "id") int id) {
		...
		Client client = prospectServices.getClientById(id);
	...
	public String saveClient(Model model, //
		...
		if (client.getName().length() > 0 && client.getSiret().length() > 0 && client.getAddress().length() > 0 && client.getPostCode().length() > 0 && client.getCity().length() > 0) {
			prospectServices.saveClient(client);
	...
	public String removeClient(@PathVariable(value = "id") int id) {
		...
		prospectServices.removeClientById(id);
		// On utilise redirect pour appeler une autre fonction de mapping
		return "redirect:/clientList";
	}
}
